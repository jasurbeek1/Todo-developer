const City = [
  ...
  const Position = ["React", "Nodejs", "Go", "Python",`Angularjs`, `Veujs`,`PHP`,`C`,`C#`, `C++`,`Java`];
  ...
  const getRow = ({
    ...
    return `
      <tr>
        ...
        <td>${Position}</td>
        <td>${typeposition}</td>
        <td>${salary === `` ? `tajribachi`: salary}</td>
        <td>${isMarried ? "Yes" : "No"}</td>
        <td class = "text-end">
          <button class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#pupilModal" onclick="editPupil(${id})"><i class="bi bi-pencil-square"></i> Edit</button>
...
pupilForm.addEventListener("submit", function (e) {
  ...
  openModalBtn.addEventListener("click", function () {
    sendBtn.textContent = "Add developer";
    modalTitle.textContent = "Adding developer";
  ...
  function editPupil(id) {
    ...
    selected = pupil;
    modalTitle.textContent = 'developer info editing';
  ...
  function deletePupil(id) {
    let isConfirm = confirm("Siz haqiqatdan ham ushbu dasturchini o'chirmoqchimisiz ?");
    if (isConfirm) {
